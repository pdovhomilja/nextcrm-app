...
export function DebugAiRequests() {
  ...
  const [previousCompletedIds, setPreviousCompletedIds] = useState<Set<string>>(
    new Set(),
  );
    ...
    try {
      ...
      if (result.success) {
        ...
        if (newlyCompleted.length > 0) {
          console.log(
            `Detected ${newlyCompleted.length} newly completed boards. Refreshing page...`,
          );
          router.refresh();
          toast.success(
            `${newlyCompleted.length} new board${newlyCompleted.length > 1 ? "s" : ""} created successfully!`,
          );
        }
