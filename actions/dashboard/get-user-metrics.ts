...
): Promise<{ success: boolean; data?: UserMetricsData; error?: string }> {
  try {
    ...
    const { dateRange, includeActivity, companyId } = validatedInput;
    ...
    return withCompanyAccessValidation(
      session.user.id,
      targetCompanyId,
      "board",  // Resource type (users access through board permissions)
      "metrics", // Action
      async () => {
        ...
        // Get total users in company (no manual company filtering needed - wrapper handles it!)
        ...
        // Get users with recent activity (active users) WITH EXPLICIT COMPANY FILTERING
            ...
            OR: [
              {
                assignedTasks: {
                  some: {
                    boardSection: {
                      board: {
                        companyId: targetCompanyId, // 🔒 CRITICAL: Filter tasks by company
                      },
                    },
        ...
        // Get user productivity data WITH EXPLICIT COMPANY FILTERING
    ...
    return { success: false, error: "Failed to retrieve user metrics" };
  }
}
