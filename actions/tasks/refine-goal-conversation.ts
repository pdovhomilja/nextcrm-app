...
export async function refineGoalConversation(values: z.infer<typeof RefineGoalSchema>): Promise<RefinementResult> {
  console.log('refineGoalConversation called with:', { messagesCount: values.messages?.length, language: values.language });
  ...
  console.log('Calling orchestrator with context:', { userId: session.user.id, companyId });
  ...
  console.log('Orchestrator result:', { hasToolCalls: !!result.toolCalls, hasText: !!result.text });

  // 3. Process the result
  const toolCalls = await result.toolCalls || [];
  const finalBriefTool = toolCalls.find(tool => tool.toolName === 'proposeFinalBrief');
  const clarificationTool = toolCalls.find(tool => tool.toolName === 'request_clarification');
  
  console.log('Found tool calls:', toolCalls.map(t => t.toolName));
  ...
  if (finalBriefTool) {
    ...
    return {
      isFinalBrief: false,
      content: textContent || 'Could you provide more details about your project?',
    };
  }
}