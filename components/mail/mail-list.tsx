...
export const MailList = () => {
  ...
  const [data, setData] = useState<{ emails: Email[]; total: number; error: string | null }>({ emails: [], total: 0, error: null });
  ...
  useEffect(() => {
    if (accountId && folderName) {
      const fetchEmails = async () => {
        ...
        setData({ emails: result.emails || [], total: result.total || 0, error: result.error || null });
        setIsLoading(false);
      };
      fetchEmails();
    }
  ...
  if (!accountId || !folderName) {
    return <div className="p-4 text-center text-muted-foreground">Select an account and folder to see emails.</div>;
  }
  ...
  if (data.emails.length === 0) {
    return <div className="p-4 text-center text-muted-foreground">No emails in this folder.</div>;
  }
