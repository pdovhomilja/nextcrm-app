...
export function CompanyProvider({ children }: { children: React.ReactNode }) {
  ...
  const activeCompany =
    memberships.find((m) => m.companyId === activeCompanyId)?.company || null;
  const userRole =
    ...
    try {
      ...
      if (result.success && result.memberships) {
        ...
        const validCompanyId =
          result.memberships.find((m) => m.companyId === sessionActiveId)
...
export function useCompanyMemberships() {
  const { memberships, refreshMemberships, isLoadingMemberships } =
    useCompany();
