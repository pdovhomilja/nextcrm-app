...
export function getModelForUseCase(
  useCase: "chat" | "analysis" | "realtime" | "structured",
) {
  const modelMap = {
...
export class CostMonitor {
  ...
  static generateOptimizationReport() {
    const totalCost = this.usage.reduce(
  ...
  private static generateRecommendations(usageByModel: any) {
    ...
    if (usageByModel["gpt-5"]?.percentage > 30) {
      recommendations.push(
        "Consider using GPT-5-Mini for general conversations to reduce costs",
      );
    }
    ...
    if (usageByModel["gpt-5-nano"]?.percentage < 10) {
      recommendations.push(
        "Consider using GPT-5-Nano for simple, real-time interactions",
      );
    }
...
const testQuery =
  "Find all high priority tasks assigned to the development team";
...
const testModels = async () => {
  ...
  console.log(
    `Response times: Nano: ${nanoTime}ms, Mini: ${miniTime}ms, Full: ${fullTime}ms`,
  );
};
```
