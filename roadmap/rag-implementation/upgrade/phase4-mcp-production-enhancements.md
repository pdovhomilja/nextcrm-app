...
const authenticatedMcpHandler = (serverSetup: (server: any) => void) => {
  return withMcpAuth(createMcpHandler(serverSetup), {
    // OAuth configuration
...
export class MCPErrorHandler {
  ...
  private static isCriticalError(error: any): boolean {
    ...
    return (
      criticalCodes.includes(error.code) || error.message?.includes("CRITICAL")
    );
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    healthStatus.services.mcpClientPool = {
      ...
      availableServers: Object.keys(tools).reduce(
        (acc, toolName) => {
  ...
  static generateReport(timeRange: { start: Date; end: Date }) {
    ...
    const report = {
      ...
      averageResponseTime:
        ...
        relevantMetrics.length,
      slowRequests: relevantMetrics.filter((m) => m.executionTime > 2000)
        .length,
