...
## ⚠️ CRITICAL: Database Query Security

### The Security Wrapper Limitation

**🚨 CRITICAL SECURITY ISSUE:** The Security-First Wrapper Pattern validates that users have access to a company, but **it does NOT automatically filter database queries by company**. 

**❌ This is WRONG and creates data leakage:**
...
return withCompanyAccessValidation(
  ...
  "metrics",
  async () => {
    // 🚨 SECURITY VULNERABILITY: No company filtering!
...
**✅ This is CORRECT with explicit company filtering:**
...
return withCompanyAccessValidation(
  ...
  "metrics", 
  async () => {
    // ✅ EXPLICIT COMPANY FILTERING REQUIRED
    const metrics = await db.task.groupBy({
      where: {
        boardSection: {
          board: {
...
const users = await db.user.findMany({
  where: {
    company_id: targetCompanyId, // 🔒 CRITICAL: Filter by company
    id: { in: userIds },
  },
});
```
...
```typescript
// ✅ CORRECT - When filtering by specific boardId, still include company
if (boardId) {
  where.boardSection = {
    board: {
      id: boardId,
      companyId: targetCompanyId, // 🔒 CRITICAL: Company filter even with boardId
    },
  };
}
```
...
- [ ] **🔒 CRITICAL: ALL database queries include `companyId: targetCompanyId` filtering**
- [ ] **🔒 CRITICAL: Never assumes security wrapper filters database queries automatically**
- [ ] **🔒 CRITICAL: Board-specific queries still include company filtering**
...
1. **🔒 ALWAYS add `companyId: targetCompanyId` to ALL database queries**
2. **🔒 NEVER assume security wrapper filters database queries automatically**
3. **🔒 ALWAYS filter by company even when using specific boardId/taskId**
