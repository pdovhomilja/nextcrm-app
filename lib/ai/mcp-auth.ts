...
declare global {
  var mcpTokenStorage:
    | Map<string, { userId: string; expiresAt: Date }>
    | undefined;
}

const globalTokenStorage =
  globalThis.mcpTokenStorage ??
  new Map<string, { userId: string; expiresAt: Date }>();
...
export class MCPAuthService {
  ...
  private readonly INTERNAL_SERVICE_SECRET =
    process.env.MCP_INTERNAL_SECRET || "mcp-internal-secret-key";
   ...
   */
  private async authenticateWithInternalToken(
    request: NextRequest,
    ...
    console.log("Token data lookup:", {
      ...
      tokensInStorage: Array.from(this.internalTokens.keys()).map((k) =>
        k.substring(0, 8),
      ),
    });
  ...
  async getMCPAuthHeaders(userId?: string): Promise<Record<string, string>> {
    ...
    if (sessionAuth.success) {
      ...
      const internalToken = await this.generateInternalToken(
        sessionAuth.context!.userId,
        30,
      );
      return {
