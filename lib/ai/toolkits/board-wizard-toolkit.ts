...
export const getBoardWizardToolkit = (context: AgentContext) => ({
  ...
  generateProjectBoard: tool({
    ...
    execute: async ({ refinedPrompt, role }) => {
      ...
      try {
        // Add protection against duplicate execution
        const existingBoard = await db.board.findFirst({
          where: {
            ...
            OR: [
              ...
              { description: { contains: "Generated by AI" } }
            ]
          }
        });

        if (existingBoard) {
          console.log("Duplicate board creation prevented, returning existing board:", existingBoard.id);
          return { boardId: existingBoard.id, boardName: existingBoard.name };
        }
        // 1. Call the LLM to get the structured board plan
        console.log("Starting generateObject call...");
        const openai = createOpenAI({ apiKey: process.env.OPENAI_API_KEY });
        let { object: boardPlan } = await generateObject({
...
Generate EXACTLY ONE project board with the following strict requirements:
- Return only ONE JSON object (not an array)
- ONE boardName
- ONE boardDescription
- 3-6 sections maximum
- 4-8 tasks per section maximum

DO NOT create multiple boards or variations. Create ONE comprehensive board that covers all aspects.

CRITICAL RULES:
1. Return a SINGLE JSON object matching the exact schema
2. Do not suggest alternative approaches or multiple options
3. Generate all content in the specified language if mentioned in the brief
4. Focus on ONE unified project plan, not separate sub-projects
...
Project Brief: ${refinedPrompt}`,
        ...
        if (Array.isArray(boardPlan)) {
          console.error("AI returned array instead of single object, taking first item");
          boardPlan = boardPlan[0];
        }

        if (!boardPlan.boardName || !boardPlan.sections) {
          throw new Error("Invalid board plan structure from AI");
        }

        // 3. Persist the generated plan to the database
        console.log("Creating main board...");
        const board = await createBoard({
          name: `${boardPlan.boardName} (AI Generated)`, // Add identifier
