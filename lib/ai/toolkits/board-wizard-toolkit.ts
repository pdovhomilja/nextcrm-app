...
const boardPlanSchema = z.object({
  ...
  boardDescription: z
    .string()
    .describe("A brief description of the project board."),
  sections: z.array(
    z.object({
      name: z
        .string()
...
export const getBoardWizardToolkit = (context: AgentContext) => ({
  ...
  generateProjectBoard: tool({
    description:
      "Generates and persists a complete project board with sections and tasks based on a detailed, user-approved project brief.",
    inputSchema: z.object({
      refinedPrompt: z
        .string()
        .describe("The final, user-approved project brief."),
      role: z
        .string()
        .describe('The user\'s role for context (e.g., "Product Manager").'),
    }),
    execute: async ({ refinedPrompt, role }) => {
      ...
      try {
        ...
        const { object: boardPlan } = await generateObject({
          model: aiConfig.structuredOutputModel || openai("gpt-4o-mini"),
...
CRITICAL: If the prompt contains language instructions (e.g., "Generate all content in Czech"), you MUST generate all text content (board name, descriptions, task names, task descriptions) in the specified language.
