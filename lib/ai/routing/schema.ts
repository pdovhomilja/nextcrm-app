...
export const IntentSchema = z.object({
  ...
  domain: z
    .enum(["tasks", "projects", "users", "metrics", "general"])
    .describe("The primary data domain the query pertains to."),

  complexity: z
    .enum(["simple", "complex", "multi_step"])
    .describe("The estimated complexity of the query."),

  requiredToolkits: z
    .array(
      z.enum(["taskManager", "projectAnalyzer", "userDirectory", "reporting"]),
    )
    .describe("A list of specialized toolkits needed to fulfill the request."),

  reasoningText: z
    .string()
    .describe("A brief explanation of why these classifications were chosen."),
});
