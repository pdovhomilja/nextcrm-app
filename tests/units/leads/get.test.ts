...
describe("getLead scope", () => {
  ...
  it("user out-of-scope returns null (assert miss)", async () => {
    mockUser("user", "u1");
    (prismadb.crm_Leads.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await getLead("l1");
    expect(res).toBeNull();
  ...
  it("manager returns lead detail (no OR in assert where)", async () => {
    ...
    const res = await getLead("l1");
    expect(res).toEqual({ id: "l1", firstName: "Alice" });
    const assertCall = (prismadb.crm_Leads.findFirst as ReturnType<typeof vi.fn>).mock.calls[0][0];
