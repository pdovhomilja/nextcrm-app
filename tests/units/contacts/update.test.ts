...
describe("updateContact", () => {
  ...
  it("fetches before state via findUnique with deletedAt:null", async () => {
    (prismadb.crm_Contacts.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", lastName: "Doe" });
    (prismadb.crm_Contacts.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", lastName: "Smith" });
  ...
  it("updates contact with correct fields", async () => {
    (prismadb.crm_Contacts.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1" });
    (prismadb.crm_Contacts.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", lastName: "Smith" });
  ...
  it("maps birthday_day/month/year to birthday field", async () => {
    ...
    const call = (prismadb.crm_Contacts.update as ReturnType<typeof vi.fn>).mock.calls[0][0];
  ...
  it("calls diffObjects with before and after state", async () => {
    ...
    (prismadb.crm_Contacts.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue(before);
    (prismadb.crm_Contacts.update as ReturnType<typeof vi.fn>).mockResolvedValue(after);
    await updateContact({ id: "c1", last_name: "Smith" });
