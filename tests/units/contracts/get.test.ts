...
describe("getContract scope", () => {
  ...
  it("user out-of-scope returns null (assert miss)", async () => {
    mockUser("user", "u1");
    (prismadb.crm_Contracts.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    const res = await getContract("c1");
    expect(res).toBeNull();
  ...
  it("user in-scope returns contract detail", async () => {
    mockUser("user", "u1");
    (prismadb.crm_Contracts.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1" });
    (prismadb.crm_Contracts.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "c1", title: "X" });
    const res = await getContract("c1");
  ...
  it("manager returns contract detail (no OR in assert where)", async () => {
    ...
    const res = await getContract("c1");
    expect(res).toEqual({ id: "c1", title: "X" });
    const assertCall = (prismadb.crm_Contracts.findFirst as ReturnType<typeof vi.fn>).mock.calls[0][0];
