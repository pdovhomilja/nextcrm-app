...
describe("updateContract", () => {
	...
	it("fetches before state via findUnique with deletedAt:null", async () => {
		...
		).mockResolvedValue({ id: "c1", title: "Old" });
		(
			prismadb.crm_Contracts.update as ReturnType<typeof vi.fn>
		).mockResolvedValue({ id: "c1", title: "New" });
	...
	it("updates contract with correct data and increments v", async () => {
		...
		(
			prismadb.crm_Contracts.update as ReturnType<typeof vi.fn>
		).mockResolvedValue({ id: "c1", title: "Contract" });
		...
		const call = (prismadb.crm_Contracts.update as ReturnType<typeof vi.fn>)
			.mock.calls[0][0];
