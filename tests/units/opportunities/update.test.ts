...
describe("updateOpportunity", () => {
	...
	it("fetches before state via findUnique with deletedAt:null", async () => {
		(
			prismadb.crm_Opportunities.findUnique as ReturnType<typeof vi.fn>
		).mockResolvedValue({ id: "o1", name: "Old" });
		(
			prismadb.crm_Opportunities.update as ReturnType<typeof vi.fn>
		).mockResolvedValue({ id: "o1", name: "New" });
	...
	it("parses budget and expected_revenue as floats", async () => {
		...
		const call = (prismadb.crm_Opportunities.update as ReturnType<typeof vi.fn>)
			.mock.calls[0][0];
