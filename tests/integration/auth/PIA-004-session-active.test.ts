...
describe("PIA-004 active session validates on protected endpoint", () => {
  ...
  it("get-session returns the user for a valid cookie", async () => {
    const me = await http()
  ...
  it("get-session returns null/empty for a fabricated cookie", async () => {
  ...
  it("sign-in page is reachable with a valid cookie (no redirect loop)", async () => {
  ...
  it("the signed-in user has userStatus=ACTIVE in the database", async () => {
